...
object UserCount {
  ...
  def getHdfsData(): Unit = {
    ...
    //  将hdfs文件转换为RDD
    val textRDD: RDD[String] = sc.textFile("hdfs://master:9000/user.txt")
  ...
  def dataToMysql(): Unit = {
      sqlContext.sql("select date,country,province," +
        ...
        jdbc("jdbc:mysql://localhost:3306/hxh?user=root&password=123456","user_action_result_daily",new Properties())
  }
}
